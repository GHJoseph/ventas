...
@section('contenido')
    @if($usuario_opciones[11]->Mod_Opc === 1)
        {!! Form::model($usuario, ['id' => 'usuario','method' => 'PATCH','route' => ['usuarios.update', $usuario->Cod_Usu]]) !!}
        <div class="panel panel-default">
            <div class="panel-heading" style="background-color: #1fb5ad;">
                <h3 class="panel-title" style="color:#fff;"><b>Actualización del Usuario</b></h3>
            </div>
                ...
                <div class="col-row">
                    <div class="col-md-1">
                        <div class="form-group">
                            ...
                            <input type='number' name="CodUsu" class="form-control " id="CodPac"
                                   value="{{$usuario->Cod_Usu}}" readonly>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="SelectPer" id="SelectPer" class="form-control classPersonal">
                                <option value="{{$usuario->Cod_Per}}">{{$persona->Pat_Per}} {{$persona->Mat_Per}} {{$persona->Nom_Per}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <input type='text' name="NomUsu" class="form-control" placeholder="Nombre del paciente"
                                   id="NomUsu" value="{{$usuario->Nom_Usu}}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="CodTipUsu" id="CodTipUsu" class="form-control">
                                @foreach($tipo_usuarios as $usuarios)
                                    <option @if($usuarios->Cod_Tip_Usu === $usuario->Cod_Tip_Usu) selected
                                            @endif value="{{$usuarios->Cod_Tip_Usu}}">{{$usuarios->Nom_Tip_Usu}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="CodPais" id="CodPais" class="form-control">
                                @foreach($pais as $paises)
                                    <option @if($paises->Cod_Pais === $usuario->cod_Pais) selected
                                            @endif value="{{ $paises->Cod_Pais}}">{{$paises->Nom_Pais}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="CodDep" id="CodDep" class="form-control">
                                @foreach($depart as $departamento)
                                    <option @if($departamento->Cod_Dep === $usuario->Cod_Dep) selected
                        ...
                        <div class="form-group">
                            ...
                            <select name="CodLoc" id="CodLoc" class="form-control">
                                @foreach($local as $locales)
                                    <option @if($locales->Cod_Loc === $usuario->Cod_Loc) selected
                                            @endif value="{{$locales->Cod_Loc}}">{{$locales->Nom_Loc}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
